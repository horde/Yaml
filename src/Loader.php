...
class Loader
{
     ...
     * @var array<Node>
     */
    protected array $_haveRefs = [];

    /**
     * All nodes
     * @var array<int, Node>
     */
    protected array $_allNodes = [];

    /**
     * Array of node parents
     * @var array<int, array<int>>
     */
    protected array $_allParent = [];
     ...
     * @var array<int, array<Node>>
     */
    protected array $_indentSort = [];
     ...
     *
     * @return array<string|int, mixed>  PHP version of parsed YAML.
     * @throws Exception
     ...
     *
     * @return void
     * @throws Exception|DomainException if indention contains tab
     * @throws ReflectionException
     */
     ...
     */
    protected function _getIndent(string $line): int
    {
     ...
     * @throws ReflectionException
     */
    protected function _parseLine(string $line): array
    {
        $array = [];
     ...
     * @throws ReflectionException
     */
    protected function _toType(string $value): mixed
    {
     ...
     *
     * @param mixed &$data Data to check for serialized PHP types.
     *
     * @return void
     * @throws Exception|LogicException if a class is whitelisted but
     *                                             not serializable
     * @throws ReflectionException if a class is not whitelisted
     */
    protected function _unserialize(mixed &$data): void
    {
        if (!is_string($data) || substr($data, 0, 5) != '!php/') {
            return;
        }
     ...
     */
    protected function _inlineEscape(string $inline): array
    {
        $saved_strings = [];
     ...
     * @throws ReflectionException
     */
    protected function _buildArray(): array
    {
        $trunk = [];
     ...
     */
    protected function _linkReferences(): void
    {
     ...
     * @param mixed $v   Value.
     */
    protected function _linkRef(Node $n, string|int $key, string|int|null $k = null, mixed $v = null): void
    {
        if (empty($k) && empty($v)) {
            // Look for &refs
     ...
     *
     * @param Node $node The node which you want to arrayize.
     *
     * @return void
     * @throws Exception
     * @throws ReflectionException
     */
    protected function _nodeArrayizeData(Node &$node): void
    {
        if ($node->children == true) {
            ...
            if (is_array($node->data)) {
                ...
                $this->_unserialize($node->data[$key]);
            } elseif (is_string($node->data)) {
                $this->_unserialize($node->data);
            }
        }
    }
     ...
     *
     * @param Node $z  A node whose references we wish to make real.
     */
    protected function _makeReferences(Node &$z): void
    {
        if (isset($z->ref)) {
            // It is a reference
            $key = key($z->data);
     ...
     *
     * @param array<string|int, mixed> $arr1
     * @param array<string|int, mixed> $arr2
     *
     * @return array<string|int, mixed>
     */
    protected function _array_kmerge(array $arr1, array $arr2): array
    {
